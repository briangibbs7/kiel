...
export default function ConfluenceStarred() {
  ...
  const { data: pages = [] } = useQuery({
    queryKey: ["starred-pages", user?.email],
    queryFn: async () => {
      const allPages = await base44.entities.Page.list();
      return allPages.filter((page) => page.watchers?.includes(user?.email));
    },
    enabled: !!user,
  });
      ...
      <div className="p-6 border-b border-[#1E1E1E] flex-shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-white">Starred Pages</h1>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        {filteredPages.length === 0 ? (
          <div className="bg-[#111] border border-[#1E1E1E] rounded-lg p-8 text-center">
            <Star className="w-12 h-12 mx-auto mb-3 text-[#666]" />
            <p className="text-[#999] mb-2">No starred pages yet</p>
            <p className="text-sm text-[#666]">Star pages to find them here later</p>
          </div>
        ) : (
